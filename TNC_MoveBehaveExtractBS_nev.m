 ...
 function [ContData, filenamestrB] = TNC_MoveBehaveExtractBS_nev(taskbase, filenamestr, targetName, dataRate, chan, elimTime, fpath)
...
%% LOAD EVENT DATA - specifically for troubleshooting 170112 dataset & using Boss-sorted channels:
...
%      end
    [NEV] = openNEVBS(filenamestrE,'read','nosave','nomat', 'report')                   
    dataEvents1 = NEV;
    unitNum1 = dataEvents1.Data.Spikes.Unit; 
    unitNums1 = unique(unitNum1);
    timeStamps1 = dataEvents1.Data.Spikes.Timestamps;
    unitOne = double(find(unitNum1 == 1));
    unitOneTS = double(timeStamps1(unitOne));                                    %Just extract TS for unit one for now
    
    shank.unit(1).ts = unitOneTS';                                               %save as shank.mat structure at end (like for tsd)
    shank.unit(1).inds = unitOne';
    shank.shankNum = 1;                                                          %just default for now
    
    %Open a second ch.: NOPE SEE STEPHEN'S EMAIL FROM 4.4.17ISH
%     [filenamestrE, path] = uigetfile('*.nev*','select the .nev file', fpath)               
% 
%     dataEvents2 = openNEVBS_laser(filenamestrE,'read','nosave','nomat', 'report');                    
%     
%     unitNum2 = dataEvents2.Data.Spikes.Unit; 
%     unitNums2 = unique(unitNum2);
%     timeStamps2 = dataEvents2.Data.Spikes.Timestamps;
%     unitTwo = double(find(unitNum2 == 1));
%     unitTwoTS = double(timeStamps2(unitTwo));                                    %Just extract TS for unit one for now
%     
%     shank.unit(2).ts = unitTwoTS';                                               %save as shank.mat structure at end (like for tsd)
%     shank.unit(2).inds = unitTwo';
%     shank.shankNum = 1;                                                         %just default for now
% 
...
%% COMPLETED ALL ANALYSIS. SAVE AND START OVER
    ...
    filenamestrB = strcat(newPath, filenamestrE);
     ...
     save([targetName '_NEV.mat'],'shank');                            
     disp(['saved as ' targetName '_NEV.mat']);
