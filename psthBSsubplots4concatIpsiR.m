...
function [alignVarMat] = psthBSsubplots4concatIpsiL(PopData, behavFile, units, alignVar1, alignVar2)
...
%  
        ...
        for z = 1:length(alignVar1)
            if j == 1
                alignVarMat(z,j) = alignVar1(z,j);
            else
                alignVar1 = alignVar1';
                alignVarMat(z,j+(j-1)) = alignVar1(z,j);
            end
        end
        for y = 1:length(alignVar2)
            if j == 1
                alignVarMat(y,j*2) = alignVar2(y,j);
            else 
                alignVar2 = alignVar2';
                alignVarMat(y,j*2) = alignVar2(y);
            end
        end
    end
end
