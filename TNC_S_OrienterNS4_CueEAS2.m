...
% Updated 4.27.18 to plots the NS4 signal trial-start aligned (left subplot) &
    % movement-aligned psths (also NS4 signal); 
    % thus rxn times from CSV file are lost (and those calculated from NS4 file are mostly negative.
% This file is good for plotting the former, but NOT the latter.
...
% wheelMv = wheel_sgolayF(1:10:end) - wheel_offset;    %Do this later filters it down even more
% wheelMvInds1 = 1:length(wheelMv);
% wheelMvInds = wheelMvInds1(1:10:end);                %these are the inds of the filtered wheel
...
wheel_inds_valid = wheelMvInds1(trueTrialStarts_valid(1)):wheelMvInds1(trueTrialStarts_valid(end-30));
...
threshold = 3e3;  %all 2017 data
intra_mvmt = find(smth_wheel_v>threshold & d_x_vals<30);  %crashes with the ns4 data:too big. %this is a "meaningful" movement: find the overlapping instances for smoothed velocity > 5 and the diff(timestamps) < 30 ms
figure; hold on;
...
figure; hold on; plot(x_vals(intra_mvmt(starts_clean)) , wheelMv(intra_mvmt(starts_clean)) ,'o');%plots in yellow (still fig 4)
...
[wheel_moves] = TNC_ExtTrigWins(wheelMv',intra_mvmt(starts_clean),mvmt_win); %sink.wins = csv(:,2), indexed by the starts_clean with an expanded window; ea. row shows the indexed vals from 10 backwards/25 ahead of the meaningful index
...
trueMvsPre = abs(magnitude) > 5e3;
trueMvsInds = find(trueMvsPre == 1);      %EAS: these are the indices of magnitude that are likely meaningful movements

trueMvsInds_R = find(magnitude < 5e3);    %EAS: 171012 & 171013 indices of magnitude for large/fast right mvments
trueMvsInds_L = find(magnitude > -5e3);   %EAS: 171012 & 171013 inds of mag for large/fast left mvments

[vals, inds] = sort(magnitude);           %EAS: this does nothing for me
figure;
imagesc(wheel_moves.wins(inds,:),[-5e3 5e3]); colormap(mapName);         %x is the movement vectors sorted by velocity magnitude 
...
% Using just the NS4 info:
% L_starts = trueTrialStarts_valid(trueMvsInds_L)';  
% R_starts = trueTrialStarts_valid(trueMvsInds_R)';

rxnL = LeftsTimes' - L_starts;  %YUP, can't really get valid rxn times this way
rxnR = RightsTimes' - R_starts;
...
%% Plot the L-movement trial start cue:
    ...
    [sink_tmpL]   = TNC_ExtTrigWinsEAS(tmpSmooth,L_starts(Ltimes_valid),[2000,2000]); %[-win +win], so mvment onset = ~700
...
%% Plot the trial-aligned data:
    % Plot tall L-movements' trial starts:
