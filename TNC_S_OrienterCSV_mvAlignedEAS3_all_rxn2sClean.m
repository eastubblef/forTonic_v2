...
% Updated 4.10.18 - tried to not convolve data for 171012 & 171013; 
% better just to create smoothing offset for movement alignments (see the non-clean same mfile name
...
% smooth_off = 715;     %should be this if I'm not convolving, but doesn't really work
% smooth_off = 480;     %should be this if I'm convolving
smooth_off = 0;       %should be this if I'm convolving
...
x_vals = (csv_data(:,1)-csv_data(1,1))+x_off+smooth_off; %should be this if I'm not convolving %add the NS4 x-offset (time) to the csv data's timestamps
...
startsNew = find([0 diff(intra_mvmtNew)]>10);                    %starts = inds of mvment vecs - specifically finds the large gaps between movement vectors & makes the next value = start
...
trueMvsInds_R_New = find(magnitudeNew < -50);  %EAS: indices of magnitude for large/fast right mvments
trueMvsInds_L_New = find(magnitudeNew > 50);   %EAS: inds of mag for large/fast left mvments
...
startTimesCSVZero = startTimesCsv-csv_data(1,1) + x_off + smooth_off;   %should be this if I'm convolving%THESE are the trial starts zeroed
...
wheelLfirstZero1 = wheelLfirstTimes-csv_data(1,1) + x_off + smooth_off;   %should be this if I'm convolving
...
%     alignVar1 = wheelsAll_LfirstValid;
    alignVar1 = wheelsAll_LfirstValid( wheelsAll_LfirstValid_valid);
...
%     alignVar2 = wheelsAll_RfirstValid;
    alignVar2 = wheelsAll_RfirstValid (wheelsAll_RfirstValid_valid);
    [respCSS2] = TNC_AlignRasters(tmpSmooth,PopData.session.unit(j).ts, PopData.currParams.stableTrials, alignVar2, psthWin, 1, 1); 
    shadedErrorBar(-psthWin(1):psthWin(2), respCSS2.image.psthZ, respCSS2.image.psthZe,'r');  %blue for left; for 151105 dataset = ipsi
...
%     end
    ...
    save([targetName '_bh6Rxn.mat'],'export_mvmt_data_for_psths');                            
    disp(['saved as ' targetName '_bh6Rxn.mat']);
