...
function [forHist, histVals, tss] = oneUnitAlignSpikeBS(pop, fname, varargin, threshInds)% No, this does actually plot one neuron's raster at a time!

%% EAS modified from Yttri's mfile to use after TNC_ConvertTSDtoPopDataBS 7/15/15
% One unit at a time is plotted on a raster and psth
...
% Updated for Boss-sorted data 3.15.17
...
% % fpath = '/Users/stubblefielde/Desktop/mfiles/DudmanLab/data/spikes/recordings15/Gad2_tag/150606contra/8009'; 
% hemi = 'ipsi'

fpath = '/Users/stubblefielde/Desktop/mfiles/DudmanLab/data/spikes/recordings16_17/170112';

% fname = strcat(fpath, '/', '150606_6007_pop.mat');
% behavFile = strcat(fpath, '/', '150606_6007_bh.mat');
...
% else if hemi == 'contra'
%     upperFile = fpath(end-16:end-5);
%     ext = strcat(upperFile(1:end-6), '_', file);
% 
%     end
% end
% 
...
behavName = strcat(fpath, '/', '170112NevBoss_bh.mat');
popName = strcat(fpath, '/', '170112NevBoss_pop.mat');

fname = popName;
% fname = strcat(fpath, '/', '150606_6007_pop.mat');   %For inputing filename directly
load(fname);
pop = PopData;
...
behavFile = behavName;
% behavFile = strcat(fpath, '/', '150606_6007_bh.mat'); %For inputing filename directly
load(behavFile)
behavior = ContData.behavior;

% threshInds = behavior.threshInds;  %for laser
threshInds = behavior.rewTimes;      %for other alignment
...
[~, theRank]=sort(sum(isfinite(forHist')));             %Sort such that 
forHist=forHist(fliplr(theRank),:);
