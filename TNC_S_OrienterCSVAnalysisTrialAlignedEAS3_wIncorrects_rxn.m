...
%     alignVar2 = rxnTimesR400mv(rxnTimesR400mv_valid);
   ...
   %% Plot the trial-aligned data: ALL L movements
    ...
    [sink_tmpAll_Laligned]   = TNC_ExtTrigWinsEAS(tmpSmooth,wheelsAll_LfirstValid( wheelsAll_LfirstValid_valid),[1000,2000]); %[-win +win], so mvment onset = ~700
...
%         end     %for the 170112 dataset
    ...
    [sink_tmpAll_Raligned]   = TNC_ExtTrigWinsEAS(tmpSmooth,wheelsAll_RfirstValid (wheelsAll_RfirstValid_valid),[1000,2000]); %[-win +win], so mvment onset = ~700
    if ~isfinite(sink_tmpRaligned.wins(1))               %EAS added in case a unit's numel indices ~= numEvents
        j = j+1;
    else
    unitNum = num2str(j);
...
%     alignVar2 = wheelsRfirstValid(wheelsRfirstValid_valid);
   ...
   export_mvmt_data_for_psths.rxnTimesR4to600mv = rxnTimesR400mv;
    ...
    save([targetName '_bh4Rxn.mat'],'export_mvmt_data_for_psths');                            
    disp(['saved as ' targetName '_bh4Rxn.mat']);
