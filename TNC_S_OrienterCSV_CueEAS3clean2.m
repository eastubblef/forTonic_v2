...
smth_wheel_New = abs(csv_data(:,2))'; % don't do The convolution of two vecs u (velocity) & v (row of ones padded by 0s), will be the overlap under the points as v slides across u
...
plot(x_vals , smth_wheel_New ,'LineWidth',2);   %orange; plots the smoothed wheel velocity data (all positive-going); 
...
intra_mvmtNew = find(smth_wheel_New>threshold & d_x_vals<30);  %this is a "meaningful" movement: find the overlapping instances for smoothed velocity > 5 and the diff(timestamps) < 30 ms
...
figure; hold on;
plot(x_vals , csv_data(:,2) ,'LineWidth',1);              %plots csv data in blue; inter-mvment-interval in red (fig. 4)
velMvmntsZeroed_New = x_vals(intra_mvmt);                     %pulls out the ts of CSV file for changing velocities w/ offset factored in
velMvmntsRaw_New = csv_data(intra_mvmt,2);                    %those raw velocities 
plot(x_vals(intra_mvmtNew) , csv_data(intra_mvmtNew,2) ,'.');   %plot the intra-mvment periods on top of the raw velocity (fig 4)
hold off;
...
% for i=1:size(wheel_movesNew.wins,1)
%     magnitudeNew(i) = trapz(wheel_movesNew.wins(i,mvmt_win(1):20));  %trapz(y) returns the approximate integral of y
% end                                                            %gives a per col. readout of how big ea. movement was
% magnitudeNew = magnitudeNew';                 %EAS: find the mvments that had more than 3ish vel readouts (of >10-30, ea) should be ~50 or greater
% trueMvsPreNew = abs(magnitudeNew) > 50;
% trueMvsIndsNew = find(trueMvsPreNew == 1);    %EAS: these are the indices of magnitude that are likely meaningful movements
% 
% trueMvsInds_L_New = find(magnitudeNew < -50);  %EAS: indices of magnitude for large/fast right mvments
% trueMvsInds_R_New = find(magnitudeNew > 50);   %EAS: inds of mag for large/fast left mvments
% 
% [valsNew, indsNew] = sort(magnitudeNew);         %EAS: this does nothing for me
% figure;
% imagesc(wheel_movesNew.wins(indsNew,:),[-50 50]); colormap(mapName);           %x is the movement vectors sorted by velocity magnitude 
% xlabel('vector window'); ylabel('movement vectors, sorted by vel mag');  %should be separating ipsi v contra (one is above, other below).


%% Relevant for export:
% Oldexport_mvmt_data_for_psths.times        = x_vals(intra_mvmt(starts_clean))'; %timestamps of the movement vector starts
% Oldexport_mvmt_data_for_psths.magnitude    = magnitude;
% Oldexport_mvmt_data_for_psths.mag_sort_i   = inds;
% 
% Oldexport_mvmt_data_for_psths.Lefts        = trueMvsInds_L                      %magnitude inds for L mvments
% Oldexport_mvmt_data_for_psths.Rights       = trueMvsInds_R                      %magnitude inds for R mvments
% Oldexport_mvmt_data_for_psths.LeftsTimes   = Oldexport_mvmt_data_for_psths.times(trueMvsInds_L); %timestamps of L mvments
% Oldexport_mvmt_data_for_psths.RightsTimes  = Oldexport_mvmt_data_for_psths.times(trueMvsInds_R); %timestamps of R mvments
% 
% %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% export_mvmt_data_for_psths.times        = x_vals(intra_mvmtNew(starts_cleanNew))'; %timestamps of the movement vector starts
% export_mvmt_data_for_psths.magnitude    = magnitudeNew;
% export_mvmt_data_for_psths.mag_sort_i   = indsNew;
% 
% export_mvmt_data_for_psths.Lefts        = trueMvsInds_L_New                      %magnitude inds for L mvments
% export_mvmt_data_for_psths.Rights       = trueMvsInds_R_New                      %magnitude inds for R mvments
% export_mvmt_data_for_psths.LeftsTimes   = export_mvmt_data_for_psths.times(trueMvsInds_L_New); %timestamps of L mvments
% export_mvmt_data_for_psths.RightsTimes  = export_mvmt_data_for_psths.times(trueMvsInds_R_New); %timestamps of R mvments
...
%                         wheelLfirstsAlignedInds(i) = wheelLfirstTinds(i+1); %added 2.2.18
 ...
 %% Don't use this %% Load the ns4 wheel info for velocity at ea. movement TS
% 
% wheelMvInds1 = 1:length(wheelMv_true);
% wheel_inds_valid = wheelMvInds1(trialStarts(1)):wheelMvInds1(trialStarts(end));
...
% 
% % x_valsNS4 = wheelMvInds';
% % for t = 1:length(x_valsNS4)
% %     if t == numel(x_valsNS4)
% %         break
% %     else
% %     vels(t) = max(wheelMv(t:t+1));
% %     end
% % end
% x_vals2 = wheelMvInds';
% d_x_vals2 = [0 diff(x_vals2')];                  %the diff of timestamps from ns4 wheel file
% smth_wheel_v2 = conv(abs(wheelMv) , [zeros(1,4) ones(1,5) 0] , 'same' )'; %The convolution of two vecs u (velocity) & v (row of ones padded by 0s), will be the overlap under the points as v slides across u
% figure;
% hold on; 
% plot(x_vals2 , smth_wheel_v2 ,'LineWidth',2);     %orange; plots the smoothed wheel velocity data (all positive-going); 
% 
% %% find intra movement periods
% 
% threshold2 = 1.75e3;  %all 2017 data
% intra_mvmt2 = find(smth_wheel_v2>threshold2 & d_x_vals2<55);  %crashes with the ns4 data:too big. %this is a "meaningful" movement: find the overlapping instances for smoothed velocity > 5 and the diff(timestamps) < 30 ms
% figure; hold on;
% plot(x_vals2 , wheelMv ,'LineWidth',1);                      %plots csv data in blue; inter-mvment-interval in red (fig. 4)
% 
% %% separate out into valid movements
% 
% mvmt_win2 = [10,25];
% % mvmt_win2 = [5,15];
% 
% starts2 = find([0 diff(intra_mvmt2')]>10);                   %starts = inds of mvment vecs - specifically finds the large gaps between movement vectors & makes the next value = start
% starts_valid2 = find(intra_mvmt2(starts2)>mvmt_win2(1) & intra_mvmt2(starts2)<(numel(wheelMv)-mvmt_win2(2)));  %these are the csv ts inds of first mvment onset
% starts_clean2 = starts2(starts_valid2');    
% % figure; hold on; %indices of the starts of meaningful mvments of csv file
% % plot(x_vals2'(intra_mvmt(starts_clean2)) , wheelMv(intra_mvmt2(starts_clean2)) ,'o');%plots in yellow (still fig 4)
% [wheel_moves2] = TNC_ExtTrigWins(wheelMv',intra_mvmt2(starts_clean2),mvmt_win); %sink.wins = csv(:,2), indexed by the starts_clean with an expanded window; ea. row shows the indexed vals from 10 backwards/25 ahead of the meaningful index
% 
% for i=1:size(wheel_moves2.wins,1)
%     magnitude2(i) = trapz(wheel_moves2.wins(i,mvmt_win(1):20));  %trapz(y) returns the approximate integral of y
% end                                                            %gives a per col. readout of how big ea. movement was
% magnitude2 = magnitude2';                   %EAS: find the mvments that had more than 3ish vel readouts (of >10-30, ea) should be ~50 or greater
%    
% [vals_velmv2, inds_velmv2] = sort(magnitude2);         
% trueMvsPre2 = abs(magnitude2) > 50;
% trueMvsInds2 = find(trueMvsPre == 1);    %EAS: these are the indices of magnitude that are likely meaningful movements
% 
% trueMvsInds_R2 = find(magnitude2 < -50);  %EAS: indices of magnitude for large/fast right mvments
% trueMvsInds_L2 = find(magnitude2 > 50);   %EAS: inds of mag for large/fast left mvments
% 
% export_mvmt_data_for_psths2.times        = x_vals2(intra_mvmt2(starts_clean2))'; %timestamps of the movement vector starts
% export_mvmt_data_for_psths2.magnitude    = magnitude2;
% export_mvmt_data_for_psths2.mag_sort_i   = inds_velmv2;
% 
% export_mvmt_data_for_psths2.Lefts        = trueMvsInds_L2                      %magnitude inds for L mvments
% export_mvmt_data_for_psths2.Rights       = trueMvsInds_R2                      %magnitude inds for R mvments
% export_mvmt_data_for_psths2.LeftsTimes   = export_mvmt_data_for_psths2.times(trueMvsInds_L2); %timestamps of L mvments
% export_mvmt_data_for_psths2.RightsTimes  = export_mvmt_data_for_psths2.times(trueMvsInds_R2); %timestamps of R mvments
% 
% 
% % %Assume more values for the psth_times - no overlap
% wheelALL_LfirstValid =  export_mvmt_data_for_psths.wheelALL_LfirstValid;
% vel3 = find(export_mvmt_data_for_psths2.LeftsTimes(1:length(wheelALL_LfirstValid)) < wheelALL_LfirstValid + 50);
% vel4 = find(export_mvmt_data_for_psths2.LeftsTimes(1:length(wheelALL_LfirstValid)) > wheelALL_LfirstValid - 50);
%  
...
%     end
    ...
    save([targetName '_bhCue.mat'],'export_mvmt_data_for_psths');                            
    disp(['saved as ' targetName '_bhCue.mat']);
