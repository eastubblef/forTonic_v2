 ...
 function [ContData, filenamestrB] = TNC_MoveBehaveExtractJosh(taskbase, filenamestr, targetName, dataRate, chan, elimTime, fpath)
...
%     figure; hold on; plot(diffLaserTSsec, 'k');
%     figure; hold on; plot(laserTSms, 0.1e4, 'k.');
...
%% New alignment structure from Josh 8.29.17:
% Load the csv data:                      

csv_data = dlmread('mVgatfive_2017_01_12_165817pXY.csv',',',2,0);
[data] = openNSx('/170112002.ns4');  
figure(3); hold off;
plot((csv_data(:,1)-csv_data(1,1))+107730,(csv_data(:,2)),'LineWidth',2);
hold on;  
% plot((csv_data(:,1)-csv_data(1,1))+107730,[0 diff(-csv_data(:,3)')]./2,'LineWidth',2); 
plot((csv_data(:,1)-csv_data(1,1))+107730,csv_data(:,3),'LineWidth',2);

plot(sgolayfilt((decimate(double(data.Data(3,:)),10)./25),3,101));

plot(decimate(double(data.Data(1,:)),10)./1e2);
  %key is this: (csv_data(:,1)-csv_data(1,1))+107730
  %107730 is an offset in units of ms (1/10 .ns4 sampling) where it is clear that recording turned on.
  %Next step is to upsample (interpolate the csv_data to get it in to a nice 1ms resolution trace)
