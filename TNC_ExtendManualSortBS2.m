...
% BS updated 1/4/16 - l. 138: index exceeds matrix dimensions (due to diff seg file vs. template file)
...
% BS updated 1/10/15 - ll. 98,189,196,205 249 updated loops: generate as many figs as units attmepting to template-match rather than drawing over fig.1
...
segArray = [3];     %first-pass testing with the initial 3 chunks that I've extracted (1/5/16)
seed_segs = [1];
shank_num = 1;
...
% file_Path = '/Users/stubblefielde/Desktop/mfiles/DudmanLab/data/spikes/recordings15/Vgatthree/151105/2ndpass/better/';
...
segArray = [25:26];   %for Vgatthree 151105 recording; apply over laser segment
seed_segs = [26];     %for Vgatthree 151104 recording; use 151104tag1001, 1st segment (thus, diff file names - figure out how to incorporate)
...
%     figure(1); clf;
    ...
    % find the 4 channels with the largest amplitudes                           %on the shank of interest for segment to be tested
...
%     figure(1); subplot(1,2,1); plot(template_vector,'b','LineWidth',3,'Color',[0.5 0.8 1]); title(['Extracting... ' num2str(currUnit)]); drawnow;
    figure(j); subplot(1,2,1); plot(template_vector,'b','LineWidth',3,'Color',[0.5 0.8 1]); title(['Extracting... ' num2str(currUnit)]); drawnow;
    ...
    for k=1:numel(segArray)
        ...
        for m=1:numel(sessionStruct.seg(currSeg).shank(shank_num).wfs)
            ...
            if chan_per_shank >= 3        
                ...
                R = corrcoef(template_vector',test_vector');                                       %could also return p value for corrcoef comparison
                heuristics.seg(k).c(1,m) = R(2,1);                                                 %R(2,1)
                heuristics.seg(k).d(1,m) = sum( abs(template_amps-test_amps) ) ;                   %this will be plotted on y axis (summed error, uV)
            else
...
%     figure(1); subplot(1,2,2); hold off; hist(cat_heur,2000); drawnow;
    figure(j); subplot(1,2,2); hold off; plot(cat_heur,cat_heurD,'k.','MarkerSize',1,'Color',[0.25 0.25 0.25]); axis([0 1 0 max(cat_heurD)]);  
...
%     figure(1); subplot(1,2,2); hold on; plot([userThresh userThresh],[0 max(cat_heurD)],'r',[0 1],[userThreshY userThreshY],'r','LineWidth',2); drawnow;
    figure(j); subplot(1,2,2); hold on; plot([userThresh userThresh],[0 max(cat_heurD)],'r',[0 1],[userThreshY userThreshY],'r','LineWidth',2); drawnow;
...
%             figure(2); subplot(121); plot(cat_heur,cat_heurD,'k.'); axis([0.5 1 0 200]); subplot(122); hist(cat_heur.*cat_heurD,2000); drawnow;
    ...
    tsTmp = [];
    indTmp = [];
    figure(j);  subplot(1,2,1); hold off; plot(template_vector,'b','LineWidth',3,'Color',[0.5 0.8 1]); hold on; 
    ...
    for k=1:numel(segArray)    
        ...
        figure(j);  subplot(1,2,1); plot(target_vector(k,:),'k'); drawnow;
