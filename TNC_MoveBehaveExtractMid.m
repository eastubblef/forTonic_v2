 ...
 function [ContData, filenamestrB] = TNC_MoveBehaveExtractMid(taskbase, filenamestr, targetName, dataRate, chan, elimTime, fpath)
%% This is Beth's working version of Josh's extraction code from Tonic_v2 

% UPDATED LAST:
% 6.2017   - general usability 
% 5.8.17   - called by MoveScriptNew.m for new clock alignment based on behavior clock start
...
% %      startTimesZeroCsv2 = startTimesCsv - startTimesCsv(2); 
% %      startTimesZeroCsv2 = startTimesCsv(2:end);
...
%      
% 
%% LOAD EVENT DATA (& extract the digital inputs)
%      dataEvents = openNEVfile_kian;         
%      laserTSsec = dataEvents.Data.SerialDigitalIO.TimeStampSec;
%      laserTSms = laserTSsec * 1000;                                         %convert to ms
%      diffLaserTSsec = diff(laserTSsec);                                     %ensure the stim box is set for the proper pulse on/off times (30Hz = .017s on/off cycle)
%      thrTsTmp = double(laserTSms);
% %      plot(laserTSms, 0.2e4, 'bl.');                 %plots to compare w/ NS4 file, but not the TS I want for laser/spike alignment
% 
%      laserTS = dataEvents.Data.SerialDigitalIO.TimeStamp;
%      thrTStmpRaw = double(laserTS);
%      diffLaserTS = diff(laserTS);
%      thrTStmpRaw = double(laserTS);
%      
%      laserTSRawAdjust = laserTS./30;
%      thrTStmpRawAdjust = double(laserTSRawAdjust);  %12.3.17: These are the proper TS for laser alignment
% 
...
%         sLeverData(1,:) = sgolayfilt(leverData(1,:),9,101);                   %y = sgolayfilt(x,k,f) applies Savitzky-Golay FIR smoothing filter to x. k is the polynomial order and must be < f, frame size.
     trialStrtsNSolData_sgolayF = sgolayfilt(trialStrtsNSol,9,101);             %this filter is good for oscillating baselines
     trialStrtsNSolData = trialStrtsNSol;                                       %just use the raw
...
%      startsRewNs4 = trialStrtsNSolData;     %the entire ns4 voltage trace
%      startsRewNs4 = lick;     %troubleshooting the old trial start signal

     
%% New 5.1.17: in order to (more) reliably estimate trial starts
%  xThold will be the length of time that the trial start "top of the gate" length is relevant - usually ~ 102-115 ms
%  
...
%     yTholdMin = 3200;
%     xThold = startT(2); %assuming that the session start signal is a square pulse; this is the falling edge                         
    xThold = startTholdMin;
    firstT = find(trialStrtsNSolData > yTholdMin);    %find the tall gaps = ea. trial start; need a Thold > any sol signal
    x = find(firstT > xThold);
    firstTtrial = x(2);
    firstTtrials = firstT(firstTtrial:end);           %This is the first trial start with TTL - get rid of pulses detected twice
    
    diffFirstTtrials = diff(firstTtrials);
    trialStarts1 = diffFirstTtrials > 1;
    trialStarts = firstTtrials(trialStarts1);         %These are the relevant trial starts (minus the last one)    
% figure; hold on;
% trialStartsPlot = plot(trialStarts, 5, 'k.');
% plot(laserTSms, 5.1, 'bl.');    
% set(gca,'ylim', [0 7]);

%% Find all the timestamps greater than trialStarts(1) for pop data:
    firstTrial = trialStarts(1);
    ContData.behavior.firstTrial = firstTrial;
    
    
    
    
    %% Don't need for now: 2.15.18:
%     vecT = [];
%      for i = 1:length(firstT)
%          if i+1 == length(firstT)
%              break
%          end
%          if firstT(i+1) == firstT(i) + 1                                    %generate vectors of continuous values; first will be begining of gate; last of the vector will be relevant too, moving forward (as trial start that I really want)
...
%              break
%          end
%          if vecT(i+xThold) == vecT(i) + xThold;   %This is flawed - figure it out.
...
%      allTrialsNS4old = vecT(Tinds+1);                                          %should be all of the first values of the gate
%      allTrialsNS4 = allTrialsNS4old + 100;
%      diffTrialStartsNS4 = diff(allTrialsNS4');
%      
%      diffTrialsNS4_2 = allTrialsNS4(2:end) - allTrialsNS4(2)
% 
...
%     forCompareNS4 = diffRefinedTrialStartsNS4(1:13);
%     forCompareCSV = diffStrtsCSV(1:13); 
%     forCompareCSV2 = diffStrtsCSV(2:13);
%     
...
%     else if compareDiffs(2) == 1
%             trialStartsZeroNS4 = refinedTrialStartsNS4(2:end)' - refinedTrialStartsNS4(2)';  %if the first trials are off, try the second trial
% %             trialStrts = [];
% %                 for i = 1:length(startTimesZeroCsv2)
...
% %                         else
% %                             if trialStartsZeroNS4(j) > startTimesZeroCsv2(i)-50 && trialStartsZeroNS4(j) < startTimesZeroCsv2(i)+50; % creates a window to search within 40 ms +/-
...
% %                     end
% %                 end
%             startTimesZeroCsv = startTimesZeroCsv2-startTimesZeroCsv2(1);
%         end
%     end
% 
...
%     ContData.behavior.trialStarts = trialStarts(1:25);
% 
%     
