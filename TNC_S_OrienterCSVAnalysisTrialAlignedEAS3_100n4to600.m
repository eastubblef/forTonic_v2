...
% U[dated 2.28.18 to adjust the reaction times to pull out (now, < 100; 400-600); saves as bh2 file.
...
% if filestr == '/170112002.ns4'
%     numUnits = numel( PopData.session(1).unit(1) );  %only use unit 1
%     figure;
% 
% else numUnits = numel( PopData.session(1).unit);
%     figure; 
% end
...
%     plot((sink_tmpR.avg - mean(sink_tmpL.avg(1:500))) + (ones(1,numel(sink_tmpR.avg)).*j.*0.01), 'r'); hold on;  
    ...
    unitNum = num2str(j);
    unitname = strcat(' u#', unitNum, 'rxn < 100 ms');
...
%     plot((sink_tmpR.avg - mean(sink_tmpL.avg(1:500))) + (ones(1,numel(sink_tmpR.avg)).*j.*0.01), 'r'); hold on;  
    ...
    unitNum = num2str(j);
    unitname = strcat(' u#', unitNum, 'rxn = 400-600 ms');
...
%     alignVar2 = rxnTimesR400mv(rxnTimesR400mv_valid);
   ...
   export_mvmt_data_for_psths.rxnTimesL100mv = rxnTimesL200mv;
...
%     end  
     ...
     save([targetName '_bh2.mat'],'export_mvmt_data_for_psths');                            
     disp(['saved as ' targetName '_bh2.mat']);
