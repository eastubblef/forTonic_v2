function [ContData, filenamestrB] = TNC_MoverBehaviorExtractBS_nev(filenamestr, targetName, dataRate, chan, fpath)
...
%  updated last 3.8.17, 7.16.15; 11.26.15; 1.14.16

%  Note: removed the following vars since that is empty in my Blackrock system (7.16.15):
%  chan.x, chan.rew, 
%  ContData.behavior.threshInds = TS for laser pulse-on (divide by 30 = 30kHz = sampling frequency for spikes) = ch. 130 (Ain2)
%  RE-ESTABLISH ACTIVE INPUTS FOR 11.26.15 DATA (1.14.16): chan.rew (129), and motor in = chan.y (131)
...
%      end
    ...
    dataEvents = openNEVBS(filenamestrE,'read','nosave','nomat', 'report');                    

%     rewIndsTmp = find(dataEvents.Data.Spikes.Electrode==chan.rew);                               %reward 
%         % eliminate any pulses that were detected twice
%         rewTsTmp = round(dataEvents.Data.Spikes.Timestamps(rewIndsTmp)./30);                     %wasn't an active input until 4.16
%         validRewInds = find(diff(rewTsTmp)>200);
% %           validRewInds = find(diff(rewTsTmp)>400);
% 
...
% 
        ...
        validThrInds = find(diff(thrTsTmp)>20);
%         validThrInds = find(diff(thrTsTmp)>40);
        % write valid data to structure
        ContData.behavior.threshInds = thrTsTmp(validThrInds);                                   %actual TSs of laser channel in

%     disp(['Number of rewards delivered: ' num2str(numel(rewTsTmp)) ' inds)']); 
%     disp(' ');disp(' '); 
    
    
%     if ~exist(chan.lick)
%         ContData.behavior.lickInds = NaN;
%         
%     else if exist(chan.lick)
%             if ~any(find(dataEvents.Data.Spikes.Electrode==chan.lick));                              %licking - loop to accomodate 11.15 data
%                 ContData.behavior.lickInds = NaN;
%             else
%                 lickInds = find(dataEvents.Data.Spikes.Electrode==chan.lick);
...
% end
%     
%     sLeverData(1,:) = sgolayfilt(leverData(1,:),9,101);  %motor
