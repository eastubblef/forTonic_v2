function [h, udata] = scalebarX(varargin)
...
% PROCESS ARGUMENTS
    ...
    % CHECK IF DATA ASPECT RATIO IS [1 1 1]
...
%     end
    ...
    linepos = [anchor; anchor];   %Possibly here
...
%     line(ends(:,1,1), ends(:,2,1), 'color', colour, 'linewidth', linewidth, 'parent', hg); %gets rid of the flipped end
...
%     text(linepos(2,1),linepos(2,2),0,[num2str(scalelength) '1'],'verticalalignment',textalignment{1},'horizontalalignment',textalignment{2}, 'color', colour, 'fontweight', fontweight, 'parent', hg);
% 
