function [response] = TNC_AlignRasters(delta,spkStamps,stableTrials,alignStamps,window,rasterFlag,boxcar)
...
%Here is where the mean of response taken across trials, or across each row
if stableTrials==-1
    response.image.alignedtime    = -window(1,1):1:window(1,2);
    response.image.psthAVG        = mean(response.image.aligned,1);% - meanRate; averages across the rows
...
%% Changed for raw data output  (no z-scoring)
% use the entire left window as an estimate of the mean

avgRate = mean(response.image.psthAVG(1:round(window(1,1)./2)));
% avgRate = mean(response.image.psthAVG(1:round(window(1,1))));
% avgRate = mean(response.image.psthAVG);
