 ...
 function [ContData, filenamestrB] = TNC_MoveBehaveExtractNew2(taskbase, filenamestr, targetName, dataRate, chan, elimTime, fpath)
%% This is Beth's version of Josh's extraction code from Tonic_v2 

% UPDATED LAST: 
% 3.21.16 - 3.29.16 use taskbase file to align TS of spikes to trial start & L v R movements (behaveLoad4)
% 3.2.16 to incorporate behavLoad.m for loading behavioral data from Beth's (updated) "taskbase" structure, based on csv file
...
%% Load event data 
    ...
    startsRewTsRoundTmp = round(dataEvents.Data.Spikes.Timestamps(startsRewIndsTmp(2:end))./30);     % This is the value to compare - don't ./30 yet.     
    ...
    for i = 1:length(startsRewsZeroCsv)
        for j = 1:length(startsRewZeroNev)
            if i == 1
                ...
                if startsRewZeroNev(j) > startsRewsZeroCsv(i)-40 && startsRewZeroNev(j) < startsRewsZeroCsv(i)+40; % creates a window to search within 40 ms +/-
...
%   if isempty(wheelILfirstsAligned)
%     wheelLfirstsAligned = [];
%     for i = 1:length(LrewTrialStartsZeroed)
